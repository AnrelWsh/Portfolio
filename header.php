<body id="body">
    <header>
        <section class="intro">
        <h1 id="ttlIntro">ANGEL HMELI'S WEB PORTFOLIO</h1>
        <img id="logoIntro" src="img/anrelgif.gif" alt="Anrel Gif"></img>
        </section>

        <div id="divLoading">
            <img class="loading" src="img/anrelgif.gif" alt="Loading Logo ">
        </div>

        <nav id="navbar">
        <a href="#accueil">
            <img src="img/anrelgif.gif" alt="Logo Anrel">
        </a>
        <div id="menu-btn" class="menu-btn-close">
            <div></div>
            <div></div>
            <div></div>
        </div>
        <div id="menu">
            <div class="menu-nav">
            <div class="sl">
                <div id="slD1" ></div>
                <a href="index.php">
                <h3 class="select-menu"> Who am I? </h3>
                </a>
            </div>
            <div class="sl">
                <div id="slD2"></div>
                <a href="skills.php">
                <h3 class="select-menu"> My skills </h3>
                </a>
            </div>
            <div class="sl">
                <div id="slD3"></div>
                <a href="projects.php">
                <h3 class="select-menu"> My projects </h3>
                </a>
            </div>
            </div>
        </div>
        </nav>

        <div class="slide-title">
        <p>ANGEL'S WEB PORTFOLIO</p>
        <p class="slide-title-reverse">ANGEL'S WEB PORTFOLIO</p>
        </div>
    </header>